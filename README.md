...
- 查看笔记：在「笔记列表」核对已保存文件、更新时间和大小，按文件名或文件夹筛选；无需模型凭证。
- 网页建库：点击「建立 / 更新索引」，查看处理状态与片段数，失败后可以重试。
- 长文分块：按 Markdown 标题划分后，再限制每块最多 1500 字符、最多 150 字符重叠，保留标题和来源；这是字符限制，不保证适配所有模型的 token 上限。
...
- 统一日报通过 `status` 区分 `ok`、`partial` 与 `failed`，正文保留失败或跳过提示，并传播 B 站采集和逐条处理的失败状态。B 站独立日报返回实际成功、失败、跳过数和明细；超出处理上限的条目列为待处理，不计入跳过数。HTTP 200 不代表所有平台成功；微信内部逐条处理的失败仍需继续完善，这不是全链路成功率指标。
