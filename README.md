...
- 统一日报通过 `status` 区分 `ok`、`partial` 与 `failed`，正文保留失败或跳过提示，并传播 B 站和微信逐条处理的失败状态。两个平台的独立日报返回实际成功、失败数和明细，B 站另有跳过数；超出处理上限的条目不计入失败数。HTTP 200 不代表所有平台成功，真实平台和模型链路仍需自行验收。
