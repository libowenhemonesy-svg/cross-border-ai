...
## 2026-09-14：网页索引与自动验证

- 44 项后端测试通过，覆盖网页索引状态、重复请求、失败重试、旧分块清理、模型独立配置、连接恢复与实时健康状态。
- GitHub Linux / Python 3.10 环境依赖安装、测试与编译检查通过。
- GitHub Node 20 环境 npm ci、前端生产构建通过。
- 两个应用的 Docker 镜像构建通过；启动 Qdrant、后端和前端后，健康检查、索引状态接口与首页均通过检查。
- [完整 CI 运行证据](https://github.com/libowenhemonesy-svg/cross-border-ai/actions/runs/34855853153)。容器检查使用无效的测试占位凭证，不调用真实模型，不能证明实际问答质量。
- 浏览器确认网页索引运行状态、缺少凭证时的失败提示和重试按钮恢复。
- 仍待验证：真实内容整理、Embedding、问答与来源核对；真实录屏；许可证选择与历史凭证轮换。
